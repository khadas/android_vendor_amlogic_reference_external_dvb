...
AM_ErrorCode_t AM_Get_XDSDataInfo(AM_XDSInfo_t *returnInfo)
{
	...
	for (i = 0; i < 8; i++)
		strcpy(returnInfo->progDesc[i].title, xdsContext->XDSBuffer->progDesc[i].title);
	...
	for (i = 0; i < 8; i++)
		strcpy(returnInfo->futureProgDesc[i].title, xdsContext->XDSBuffer->futureProgDesc[i].title);
...
void AM_Initialize_XDSData(AM_XDSContext_t *context)
{
	...
	for (i = 0; i < 8; i++)
		strcpy(context->XDSBuffer->futureProgDesc[i].title, " ");
...
AM_ErrorCode_t AM_Decode_XDSData(UINT8 data1, UINT8 data2)
{
			 ...
			 data1 == 0x0A || data1 == 0x0C || data1 == 0x0E)
	{
		 ...
		 * valid xds data.So  reset everything. */
...
static void xds_handle_PINstate(UINT8 data, AM_XDSContext_t *context)
{
	...
	case 1:
		...
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->startTime.minute = tempTime.minute;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureStartTime.minute = tempTime.minute;
	...
	case 2:
		...
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->startTime.hour = tempTime.hour;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureStartTime.hour = tempTime.hour;
	...
	case 4:
		month = data & 0xF;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->startMonth = month;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureStartMonth = month;
...
static void xds_handle_lentimestate(UINT8 data, AM_XDSContext_t *context)
{
	...
	case 1:
		...
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->progLen.minute = temp.minute;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureProgLen.minute = temp.minute;
	...
	case 2:
		temp.hour = data & 0x3F;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->progLen.hour = temp.hour;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureProgLen.hour = temp.hour;
	...
	case 3:
		temp.minute = data & 0x3F;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->timeElapsed.minute = temp.minute;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureTimeElapsed.minute = temp.minute;
	...
	case 4:
		temp.hour = data & 0x3F;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->timeElapsed.hour = temp.hour;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureTimeElapsed.hour = temp.hour;
	...
	case 5:
		x = data & 0x3F;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->timeElapsedSec = x;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureTimeElapsedSec = x;
...
static void xds_handle_aspectratiostate(UINT8 data, AM_XDSContext_t *context)
{
	...
	case 1:
		temp = (data & 0x3F) + 22;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->aspectRatio.startLine = temp;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureAspectRatio.startLine = temp;
...
static void xds_handle_cgmsstate(UINT8 data, AM_XDSContext_t *context)
{
	...
	case 1:
		...
		tData = data & 0x1;
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->cgmsInfo.analogSourceBit = tData;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futureCgmsInfo.analogSourceBit = tData;
...
static AM_ErrorCode_t xds_finish_comppkt1state(AM_XDSContext_t *context)
{
	...
	case 2:
		...
		if (len <= 4)
			...
			numTypes = 5; /* if more than 4 type chars received, have  the maximum number - 5 */
...
static void xds_handle_comppkt2state(UINT8 data, AM_XDSContext_t *context)
{
	...
	case 1:
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->pkt2.startTime.minute = data & 0x3F;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futurePkt2.startTime.minute = data & 0x3F;
		break;
	case 2:
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->pkt2.startTime.hour = data & 0x1F;
		else if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->futurePkt2.startTime.hour = data & 0x1F;
		break;

	case 3:
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->pkt2.startDate = data & 0x1F;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futurePkt2.startDate = data & 0x1F;
		break;

	case 4:
		if (context->currentXDSClass == CURRENTCLASSSTATE)
			info->pkt2.startMonth = data & 0xF;
		else if (context->currentXDSClass == FUTURECLASSSTATE)
			info->futurePkt2.startMonth = data & 0xF;
		break;
...
static void xds_handle_nativechannelstate(UINT8 data, AM_XDSContext_t *context)
{
	...
	if (data >= 0x20 && data <= 0x7F)
	{
		...
		if (context->nativeChannelCount <= 4)
		{
			...
			if (context->nativeChannelCount != 4)
				context->station.name[index + 1] = '\0';
		}
...
static AM_XDSProgRating_t xds_get_celprograting(UINT8 data)
{
	...
	case 2:
		return CE_CHILDREN8;
	case 3:
		return CE_GENERALPROGRAMMING;
	case 4:
		return CE_PARENTALGUIDANCE;
	case 5:
		return CE_FOURTEENPLUS;
	case 6:
		return CE_EIGHTEENPLUS;
	default:
		return RATINGNOTAVAILABLE;
	}
}
...
static AM_XDSProgRating_t xds_get_cflprograting(UINT8 data)
{
	...
	case 2:
		return CF_EIGHTPLUS;
	case 3:
		return CF_THIRTEENPLUS;
	case 4:
		return CF_SIXTEENPLUS;
	case 5:
		return CF_EIGHTEENPLUS;
	default:
		return RATINGNOTAVAILABLE;
	}
}
...
static AM_XDSAudioType_t xds_get_mainaudio(UINT8 data)
{
	...
	case 1:
		return MONO;
	case 2:
		return SIMULATEDSTEREO;
	case 3:
		return TRUESTEREO;
	case 4:
		return STEREOSURROUND;
	case 5:
		return DATASERVICE;
	case 6:
		return OTHERTYPE;
...
static const char *xds_get_programratingstring(AM_XDSProgRating_t rating)
{
	...
	case CE_CHILDREN:
		return "children";
	case CE_CHILDREN8:
		return "children eight plus";
	case CE_FOURTEENPLUS:
		return "fourteen plus";

	case CE_EIGHTEENPLUS:
	case CF_EIGHTEENPLUS:
		return "eighteen plus";

	case CF_EIGHTPLUS:
		return "eight plus";
	case CF_THIRTEENPLUS:
		return "thirteen plus";
	case CF_SIXTEENPLUS:
		return "sixteen plus";
	default:
		return "noAvailable";
...
static const char *xds_get_programtypestring(AM_XDSProgType_t type)
{
	...
	case AM_XDSPROG_OTHERS:
		return "others";
	case AM_XDSPROG_ACTION:
		return "action";
	case AM_XDSPROG_ADVERTISEMENT:
		return "advertisement";
	case AM_XDSPROG_ANIMATED:
		return "animated";
	case AM_XDSPROG_ANTHOLOGY:
		return "anthology";
	case AM_XDSPROG_AUTOMOBILE:
		return "automobile";
	case AM_XDSPROG_AWARDS:
		return "awards";
	case AM_XDSPROG_BASEBALL:
		return "baseball";
	case AM_XDSPROG_BASKETBALL:
		return "basketball";
	case AM_XDSPROG_BULLETIN:
		return "bulletin";
	case AM_XDSPROG_BUSINESS:
		return "business";
	case AM_XDSPROG_CLASSICAL:
		return "classical";
	case AM_XDSPROG_COLLEGE:
		return "college";
	case AM_XDSPROG_COMBAT:
		return "combat";
	case AM_XDSPROG_COMEDY:
		return "comedy";
	case AM_XDSPROG_COMMENTARY:
		return "commentary";
	case AM_XDSPROG_CONCERT:
		return "concert";
	case AM_XDSPROG_CONSUMER:
		return "consumer";
	case AM_XDSPROG_CONTEMPORARY:
		return "contemporary";
	case AM_XDSPROG_CRIME:
		return "crime";
	case AM_XDSPROG_DANCE:
		return "dance";
	case AM_XDSPROG_DOCUMENTARY:
		return "documentary";
	case AM_XDSPROG_DRAMA:
		return "drama";
	case AM_XDSPROG_ELEMENTARY:
		return "elementary";
	case AM_XDSPROG_EROTICA:
		return "erotica";
	case AM_XDSPROG_EXERCISE:
		return "exercise";
	case AM_XDSPROG_FANTASY:
		return "fantasy";
	case AM_XDSPROG_FARM:
		return "farm";
	case AM_XDSPROG_FASHION:
		return "fashion";
	case AM_XDSPROG_FICTION:
		return "fiction";
	case AM_XDSPROG_FOOD:
		return "food";
	case AM_XDSPROG_FOOTBALL:
		return "football";
	case AM_XDSPROG_FOREIGN:
		return "foreign";
	case AM_XDSPROG_FUNDRAISER:
		return "fundRaiser";
	case AM_XDSPROG_GAMEQUIZ:
		return "gameQuiz";
	case AM_XDSPROG_GARDEN:
		return "garden";
	case AM_XDSPROG_GOLF:
		return "golf";
	case AM_XDSPROG_GOVERNMENT:
		return "government";
	case AM_XDSPROG_HEALTH:
		return "health";
	case AM_XDSPROG_HIGHSCHOOL:
		return "highSchool";
	case AM_XDSPROG_HISTORY:
		return "history";
	case AM_XDSPROG_HOBBY:
		return "hobby";
	case AM_XDSPROG_HOCKEY:
		return "hockey";
	case AM_XDSPROG_HOME:
		return "home";
	case AM_XDSPROG_HORROR:
		return "horror";
	case AM_XDSPROG_INFORMATION:
		return "information";
	case AM_XDSPROG_INSTRUCTION:
		return "instruction";
	case AM_XDSPROG_INTERNATIONAL:
		return "international";
	case AM_XDSPROG_INTERVIEW:
		return "interview";
	case AM_XDSPROG_LANGUAGE:
		return "language";
	case AM_XDSPROG_LEGAL:
		return "legal";
	case AM_XDSPROG_LIVE:
		return "live";
	case AM_XDSPROG_LOCAL:
		return "local";
	case AM_XDSPROG_MEDICAL:
		return "medical";
	case AM_XDSPROG_MEETING:
		return "meeting";
	case AM_XDSPROG_MILITARY:
		return "military";
	case AM_XDSPROG_MINISERIES:
		return "miniseries";
	case AM_XDSPROG_MUSIC:
		return "music";
	case AM_XDSPROG_MYSTERY:
		return "mystery";
	case AM_XDSPROG_NATIONAL:
		return "national";
	case AM_XDSPROG_NATURE:
		return "nature";
	case AM_XDSPROG_POLICE:
		return "police";
	case AM_XDSPROG_POLITICS:
		return "politics";
	case AM_XDSPROG_PREMIERE:
		return "premiere";
	case AM_XDSPROG_PRERECORDED:
		return "prerecorded";
	case AM_XDSPROG_PRODUCT:
		return "product";
	case AM_XDSPROG_PROFESSIONAL:
		return "professional";
	case AM_XDSPROG_PUBLICGR:
		return "publicGr";
	case AM_XDSPROG_RACING:
		return "racing";
	case AM_XDSPROG_READING:
		return "reading";
	case AM_XDSPROG_REPAIR:
		return "repair";
	case AM_XDSPROG_REPEAT:
		return "repeat";
	case AM_XDSPROG_REVIEW:
		return "review";
	case AM_XDSPROG_ROMANCE:
		return "romance";
	case AM_XDSPROG_SCIENCE:
		return "science";
	case AM_XDSPROG_SERIES:
		return "series";
	case AM_XDSPROG_SERVICE:
		return "service";
	case AM_XDSPROG_SHOPPING:
		return "shopping";
	case AM_XDSPROG_SOAPOPERA:
		return "soapOpera";
	case AM_XDSPROG_SPECIAL:
		return "special";
	case AM_XDSPROG_SUSPENSE:
		return "suspense";
	case AM_XDSPROG_TALK:
		return "talk";
	case AM_XDSPROG_TECHNICAL:
		return "technical";
	case AM_XDSPROG_TENNIS:
		return "tennis";
	case AM_XDSPROG_TRAVEL:
		return "travel";
	case AM_XDSPROG_VARIETY:
		return "variety";
	case AM_XDSPROG_VIDEO:
		return "video";
	case AM_XDSPROG_WEATHER:
		return "weather";
	case AM_XDSPROG_WESTERN:
		return "western";
	default:
		return "notAvailable";
	}
...
static const char *xds_get_cgmsstring(AM_CgmsInfo_t type)
{
	...
	case ONEGENERATIONCOPY:
		return "oneGenerationCopy";
	case COPYNOTPERMITTED:
		return "copyNotPermitted";
	case NOAPS:
		return "noAPS";
	case PSPONSPLITBURSTOFF:
		return "pspOnSplitBurstOff";
	case PSPON2LINESPLITBURSTON:
		return "pspOn2LineSplitBurstOn";
	case PSPON4LINESPLITBURSTON:
		return "pspOn4LineSplitBurstOn";
	default:
		return "notAvailable";
	}
}
...
/*
** add api for invade access dvb.db to store xds items information
...
static AM_ErrorCode_t xds_get_svr_si_table_items(sqlite3 *sqldb, int *db_ts_id, int *srv_db_id, int *source_id, int *src)
{
	...
	snprintf(sql, sizeof(sql), "select db_id,src from ts_table where chan_num=%d",  ntsc_current_channumner);//need APK set correct virtual channel number
...
#else
	// directly use chan_num like 0x20000 means major number is 2,minor number is 0
	...
	if (AM_DB_Select(sqldb, sql, &row, "%d,%d,%d,%s:256,%d,%d,%d,%d", &srv_dbid, &src_id, &ts_dbid, namestring, &major_chan_num,
					 &minor_chan_num, &default_chan_num, &tmp_source_id) != AM_SUCCESS ||
		row == 0)
	{
		/*No such service*/
		AM_XDS_DBG("\n[%s:%d]==== cannot get current srv_table\n", __FUNCTION__, __LINE__);
...
static AM_ErrorCode_t xds_insert_evt_table_items(sqlite3 *sqldb, int src, int ts_dbid, int srv_dbid, int source_id)
{
	...
	AM_XDS_DBG("\n==== insert new record into evt_table,sqldb:%p,src:%d,db_ts_id:%d,srv_db_id:%d,source_id:%d\n", sqldb, src, ts_dbid, srv_dbid, source_id);
...
static AM_ErrorCode_t xds_update_evt_table_items(sqlite3 *sqldb, AM_xdsUserProcEvent_t event, int ts_dbid, int srv_dbid, int evt_dbid, int source_id)
{
	...
	case XDSPROGRAMDESCRIPTIONEVENT: //"update evt_table set descr=?,items=?,ext_descr=? where db_id=?" //now reserve for future use if in need.
	...
	default:
		AM_XDS_DBG("\n[%s] not support process event :%d save into database!\n", __FUNCTION__, event);
		break;
	}
...
static AM_ErrorCode_t xds_store_useproc_event(AM_xdsUserProcEvent_t event)
{
	...
	if (AM_DB_Select(sqldb, sql, &row, "%d,%d,%s:256,%d,%d,%s:256,%s:256,%d,%s:256", &evt_dbid, &event_id, event_name, &starttime, &endtime, event_descr,
					 event_ext_descr, &source_id, rrt_rating) == AM_SUCCESS &&
		row > 0)
	{
		need_update = AM_TRUE;
		AM_XDS_DBG("\n==== current evt_dbid : (%d),event_id:%d,starttime:%d,endtime:%d\n", evt_dbid, event_id, starttime, endtime);
		AM_XDS_DBG("\n==== current evt_name: (%s)!\n", event_name);
	...
	else
	{
		if (0 == clear_expired_evt_table)
		{
			...
			if (AM_SUCCESS != ret)
			{
				AM_XDS_DBG("\n[%s:%d] fail to clear expired event in evt_table items\n", __FUNCTION__, __LINE__);
			}
...
void AM_Dump_XDSDataInfo(AM_XDSInfo_t *info)
{
	...
	if (info->rating.fantasyViolence)
		AM_XDS_DBG("[FantasyViolence]");
	if (info->rating.violence)
		AM_XDS_DBG("[Violence]");
	if (info->rating.sexualSituations)
		AM_XDS_DBG("[SexualSituations]");
	if (info->rating.adultLanguage)
		AM_XDS_DBG("[AdultLanguage]");
	if (info->rating.ssDialog)
		AM_XDS_DBG("[SexuallySuggestiveDialog]");
	AM_XDS_DBG("\n");
	...
	if (info->futureRating.fantasyViolence)
		AM_XDS_DBG("[FantasyViolence]");
	if (info->futureRating.violence)
		AM_XDS_DBG("[Violence]");
	if (info->futureRating.sexualSituations)
		AM_XDS_DBG("[SexualSituations]");
	if (info->futureRating.adultLanguage)
		AM_XDS_DBG("[AdultLanguage]");
	if (info->futureRating.ssDialog)
		AM_XDS_DBG("[SexuallySuggestiveDialog]");
	AM_XDS_DBG("\n");
